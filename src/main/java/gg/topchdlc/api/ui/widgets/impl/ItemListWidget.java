...
import gg.topchdlc.vse.shutki.module.settings.impl.itemlist.ItemListRenderer;
...
public class ItemListWidget extends UIWidget {

    private final ItemListRenderer renderer;
    private final Drag drag = new Drag("ItemList", () -> true);
    ...
    public ItemListWidget(ItemListRenderer renderer) {
    ...
    public void syncRenderers() {
        rows.clear();
        for (var id : renderer.getSetting().getItems()) {
            Item item = Registries.ITEM.get(id);
            if (item != null && item != Items.AIR) {
                rows.add(new ItemRow(item, renderer.getSetting()));
            }
        }
    }
    ...
    public void render(int mouseX, int mouseY) {
        ...
        rows.removeIf(ItemRow::shouldRemove);
            ...
            if (MathUtility.mouseIn(x, y + 52, width, height - 62, mouseX, mouseY)) {
                for (Map.Entry<Item, Rectangle> entry : hitboxes.entrySet()) {
                    if (entry.getValue().hovered(mouseX, mouseY)) {
                        ...
                        rows.add(new ItemRow(entry.getKey(), renderer.getSetting()));
                        closeAdding();
                        return true;
                    }
                }
    ...
    private static class ItemRow extends RendererObject {
        private final Item item;
        private final ItemListSetting setting;
        private boolean remove = false;

        ItemRow(Item item, ItemListSetting setting) {
            this.item = item;
        ...
        public void render(int mouseX, int mouseY) {
            ...
            sys.alpha(originAlpha);
            try {
                Client.RENDERER.texture(item, x + 6, y + 2, 16, 16, 1,
                        new Vector4f(0), Color.WHITE, Color.WHITE, Color.WHITE, Color.WHITE);
            } catch (Exception ignored) {}
