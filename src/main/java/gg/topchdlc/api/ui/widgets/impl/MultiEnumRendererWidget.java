...
import gg.topchdlc.vse.shutki.module.settings.impl.multienum.MultiEnumRenderer;
 ...
 */
public class MultiEnumRendererWidget extends UIWidget {
    private final MultiEnumRenderer<?> parent;
    private final Animation animation = new Animation();
    private boolean closing = false;
    private float scroll = 0f;
    private float scrollAnim = 0f;

    public MultiEnumRendererWidget(MultiEnumRenderer<?> parent, float targetX, float targetY, float targetWidth) {
        this.parent = parent;
        this.x = targetX;
        this.y = targetY;
    ...
    public void render(int mouseX, int mouseY) {
        ...
        List<?> entries = parent.getSetting().getEnumEntries();
        float itemHeight = 13f;
        float totalContentHeight = entries.size() * itemHeight + 3f;
        ...
        if (progress > 0.15f) {
            ...
            for (int i = 0; i < entries.size(); i++) {
                Enum<?> entry = (Enum<?>) entries.get(i);
                String name = LangUtility.getEnumChoiceName(entry);
                boolean isSelected = parent.getSetting().get().contains(entry);
                ...
                if (isSelected) {
                    ...
                    itemBg = new Color(accent.getRed(), accent.getGreen(), accent.getBlue(), (int) (40 * progress));
                } else if (hovered) {
                ...
                Client.RENDERER.text(name, x + 5f, textY, TextureUse.SFMEDIUM, textFontSize, textColor);

                IconUse icon = isSelected ? IconUse.ONMODULE : IconUse.OFFMODULE;
                Color iconColor = isSelected ? ClientSettings.INSTANCE.getColor(0) : new Color(100, 100, 105, (int) (180 * progress));
                float iconSize = 5.5f;
                float iconX = x + width - 11f;
                float iconY = curY + (itemHeight - iconSize) / 2f;
                Client.RENDERER.text(icon, iconX, iconY, TextureUse.ICONS, iconSize, iconColor);
