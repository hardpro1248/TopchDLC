...
import gg.topchdlc.vse.utils.other.JavaUtility;
import gg.topchdlc.vse.utils.other.LogUtility;
import gg.topchdlc.vse.utils.math.ProfilerUtility;
...
public class Events {
    ...
    public void register(Object owner) {
        for (Method method : owner.getClass().getDeclaredMethods()) {
            if (method.getParameterCount() == 1 && Event.class.isAssignableFrom(method.getParameterTypes()[0]) && void.class.isAssignableFrom(method.getReturnType())) {
    ...
    public <E extends Event> E post(E event) {
        if (Client.IS_PANIC) return event;

        ProfilerUtility.push("event-post");
        for (Invoker invoker: events) {
            if (invoker.event.isAssignableFrom(event.getClass())) {
                try {
