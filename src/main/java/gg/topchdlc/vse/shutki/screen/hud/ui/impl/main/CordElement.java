 ...
 */
public class CordElement extends HudElement implements MinecraftHolder {
    private final MultiEnumSetting<Element> elements = settings.multiEnumSetting("Элементы", Element.COORDS);
    private final EnumSetting<Style> style = settings.enumSetting("Стиль", Style.GLASS);

    public CordElement(Drag drag) {
        super("Cord Widget", drag);
    }
    ...
    public void render(int mouseX, int mouseY) {
        ...
        for (ElementData data : activeElements) {
            ...
            Client.RENDERER.text(data.element.icon, iconX - 1.5f, iconY, TextureUse.ICONS, ICON_SIZE, Color.WHITE);
