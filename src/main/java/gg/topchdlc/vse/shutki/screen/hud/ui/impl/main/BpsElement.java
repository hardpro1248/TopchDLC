 ...
 */
public class BpsElement extends HudElement implements MinecraftHolder {
    private final MultiEnumSetting<Element> elements = settings.multiEnumSetting("Элементы", Element.BPS);
    private final EnumSetting<Style> style = settings.enumSetting("Стиль", Style.GLASS);

    public BpsElement(Drag drag) {
        super("Bps Widget", drag);
    }
    ...
    public void render(int mouseX, int mouseY) {
        ...
        for (ElementData data : activeElements) {
            ...
            Client.RENDERER.text(data.element.icon, iconX, iconY, TextureUse.ICONS, ICON_SIZE, Color.WHITE);
    ...
    enum Element implements EnumChoice {
        BPS("Bps", "N", () -> {
            ...
            double bps = MoveUtility.getBPS(mc.player);
            return String.valueOf(Math.round(bps * 10f) / 10f) + " bps";
        });
        @Getter final String renderName;
        final String icon;
