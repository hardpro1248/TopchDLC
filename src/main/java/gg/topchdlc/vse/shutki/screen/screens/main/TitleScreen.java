...
public class TitleScreen extends Screen {
    ...
    private static final Identifier AVATAR_TEXTURE = Identifier.of("topchdlc", "images/ui/title/avatar.png");
    ...
    private void renderAnimatedBackground(DrawContext context, float delta) {
        if (VideoWallpaper.render(context, width, height)) return;
    ...
    protected void init() {
        ...
        float startY = height / 2f + 10f;
        float centerX = (width - BUTTON_WIDTH) / 2f;

        buttons.clear();
        buttonLabels.clear();

        addButton(centerX, startY + margin, BUTTON_WIDTH, BUTTON_HEIGHT,
                Text.translatable("menu.singleplayer").getString(),
                CustomButton.CustomButtonBuilder.ButtonType.MAIN,
                () -> client.setScreen(new SelectWorldScreen(this))
        );

        addButton(centerX, startY, BUTTON_WIDTH, BUTTON_HEIGHT,
                Text.translatable("menu.multiplayer").getString(),
                CustomButton.CustomButtonBuilder.ButtonType.MAIN,
                () -> client.setScreen(new MultiplayerScreen(this))
        );

        addButton(centerX, startY + margin * 2, BUTTON_WIDTH, BUTTON_HEIGHT,
                "Account Switcher",
        ...
        addButton(centerX, startY + margin * 3, halfButtonWidth, BUTTON_HEIGHT,
                Text.translatable("menu.options").getString(),
                CustomButton.CustomButtonBuilder.ButtonType.MAIN,
                () -> client.setScreen(new OptionsScreen(this, client.options))
        );

        addButton(centerX + halfButtonWidth + SPACING, startY + margin * 3, halfButtonWidth, BUTTON_HEIGHT,
