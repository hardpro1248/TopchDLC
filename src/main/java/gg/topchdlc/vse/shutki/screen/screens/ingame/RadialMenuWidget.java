...
public class RadialMenuWidget extends UIWidget {
    ...
    private static final float OUTER_R = 85f;
    private static final float INNER_R = 55f;
    private static final float ITEM_R = 70f;
    ...
    private float cx, cy;
    private final float[] hoverAnim = new float[MAX_SLOTS];
    private long lastTime = 0;

    public int lastHovered = -1;

    public int size() {
        return count;
    }

    public void setup(List<ItemStack> stacks, List<String> texts, Selection sel, String hintText) {
        entries.clear();
        labels.clear();
        for (int i = 0; i < stacks.size() && i < MAX_SLOTS; i++) {
            entries.add(stacks.get(i) == null ? ItemStack.EMPTY : stacks.get(i));
            labels.add(i < texts.size() ? texts.get(i) : "");
        }
        selection = sel;
    ...
    private float midDeg(int i) {
        if (count == 0) return 0;
        return -90f + i * (360f / count);
    }

    private float spread() {
        if (count == 0) return 0;
        return (360f / count) / 2f - 2f;
    }

    public boolean inSector(float mx, float my, int i) {
        ...
        float d = ((angle - midDeg(i)) % 360 + 360) % 360;
        if (d > 180) d -= 360;
    ...
    private void renderMenu(int mouseX, int mouseY) {
        ...
        for (int s = 0; s < count; s++) {
            ...
            float midRadian = (float) Math.toRadians(midDeg(s));
            float shift = anim * 8f;
            float scx = cx + (float) Math.cos(midRadian) * shift;
            float scy = cy + (float) Math.sin(midRadian) * shift;

            drawSegmentFill(scx, scy, INNER_R, OUTER_R, midDeg(s), spread(),
                    lerpColor(new Color(255, 255, 255, 28), new Color(80, 140, 255, 115), anim));
            drawSegmentOutline(scx, scy, INNER_R, OUTER_R, midDeg(s), spread(),
                    lerpColor(new Color(255, 255, 255, 120), new Color(255, 255, 255, 255), anim));

            float iconX = scx + (float) Math.cos(midRadian) * ITEM_R;
            float iconY = scy + (float) Math.sin(midRadian) * ITEM_R;

            ItemStack stack = s < entries.size() ? entries.get(s) : ItemStack.EMPTY;
            if (stack == null || stack.isEmpty()) {
                Client.RENDERER.textCentered("+", iconX, iconY - 5f, TextureUse.SFMEDIUM, 14f, new Color(255, 255, 255, 180));
            } else if (ctx != null) {
            ...
            float textX = scx + (float) Math.cos(midRadian) * (OUTER_R + 14f);
            float textY = scy + (float) Math.sin(midRadian) * (OUTER_R + 14f);
            String label = s < labels.size() ? labels.get(s) : "";
            Client.RENDERER.textCentered(label, textX, textY - 3f, TextureUse.SFMEDIUM, 5f, ClientColors.SECONDARY_FORE_COLOR);
        }

        if (!hint.isEmpty()) {
            Client.RENDERER.textCentered(hint, cx, cy + 36f, TextureUse.SFMEDIUM, 5f, new Color(200, 200, 210));
        }
    }
    ...
    public boolean click(int mouseX, int mouseY, int button) {
        ...
        for (int s = 0; s < count; s++) {
            if (inSector(mouseX, mouseY, s)) {
                if (selection != null) selection.onSelect(s);
                return true;
            }
        }
        return false;
    ...
    public void triggerRelease() {
        if (lastHovered >= 0 && lastHovered < count && selection != null) {
            selection.onSelect(lastHovered);
        }
    }
