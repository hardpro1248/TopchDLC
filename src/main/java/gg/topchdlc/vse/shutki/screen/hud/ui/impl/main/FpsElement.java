 ...
 */
public class FpsElement extends HudElement implements MinecraftHolder {
    private final MultiEnumSetting<Element> elements = settings.multiEnumSetting("Элементы", Element.FPS);
    private final EnumSetting<Style> style = settings.enumSetting("Стиль", Style.GLASS);

    public FpsElement(Drag drag) {
        super("FPS Widget", drag);
    }
    ...
    enum Element implements EnumChoice {
        FPS("FPS", "Q", () -> mc.getCurrentFps() + " fps");
        @Getter final String renderName;
        final String icon;
