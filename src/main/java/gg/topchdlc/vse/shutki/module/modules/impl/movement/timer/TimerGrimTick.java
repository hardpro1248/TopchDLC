 ...
 */
public class TimerGrimTick extends Choice {
    public TimerGrimTick(){super("TimerGrimTick");}

    private int ticks;
    private int groundTicks;
    private boolean isJumpHolding;
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof EventGameTick) {
            isJumpHolding = mc.options.jumpKey.isPressed();
            if (isJumpHolding) {
        ...
        if (event instanceof EventInput) {
            ...
            if (isJumpHolding && groundTicks >= 1) {
                mc.player.jump();
            }
        }
