...
import gg.topchdlc.api.ui.widgets.impl.MultiEnumRendererWidget;
...
public class MultiEnumRenderer<E extends Enum<E>> extends SettingRenderer<MultiEnumSetting<E>> {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        int selectedCount = setting.get().size();
        int totalCount = setting.getEnumEntries().size();
        String summaryText = selectedCount + "/" + totalCount;

        float valFontSize = 6.0f;
        float valTextWidth = Client.RENDERER.textWidth(summaryText, TextureUse.SFMEDIUM, valFontSize);
        float btnWidth = Math.max(34f, valTextWidth + 12f);
        float btnHeight = 11f;
        ...
        Client.RENDERER.text(summaryText, textX, textY, TextureUse.SFMEDIUM, valFontSize, Color.WHITE);
    }
    ...
    public boolean click(int mouseX, int mouseY, int button) {
        if (button == 0) {
            ...
            float valTextWidth = Client.RENDERER.textWidth(summaryText, TextureUse.SFMEDIUM, 6.0f);
            float btnWidth = Math.max(34f, valTextWidth + 12f);
            float btnHeight = 11f;
            ...
            if (MathUtility.mouseIn(btnX, btnY, btnWidth, btnHeight, mouseX, mouseY)) {
                ...
                for (E entry : setting.getEnumEntries()) {
                    String eName = LangUtility.getEnumChoiceName(entry);
                    float w = Client.RENDERER.textWidth(eName, TextureUse.SFMEDIUM, 5.8f);
                    if (w > maxTextWidth) maxTextWidth = w;
                }

                float dropWidth = Math.max(btnWidth, maxTextWidth + 26f);
                float dropX = Math.min(btnX + btnWidth - dropWidth, endX - dropWidth);

                MultiEnumRendererWidget widget = new MultiEnumRendererWidget(this, dropX, btnY + btnHeight + 2f, dropWidth);
