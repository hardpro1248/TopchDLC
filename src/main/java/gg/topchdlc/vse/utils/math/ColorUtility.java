...
public class ColorUtility {
    ...
    private final Map<Integer, Color> LINEAR_CACHE = new HashMap<>();
    private final Map<Integer, Color> GRADIENT_CACHE = new HashMap<>();
    ...
    public Color linear(Color src, Color dest, float amount) {
        ...
        int key = (src.getRGB() & 0xFFFFFF) | ((dest.getRGB() & 0xFFFFFF) << 24) | (int) (amount * 255) << 48;
        Color cached = LINEAR_CACHE.get(key);
        ...
        cached = new Color(MathHelper.clamp(red, 0, 255), MathHelper.clamp(green, 0, 255), MathHelper.clamp(blue, 0, 255), MathHelper.clamp(alpha, 0, 255));
        if (LINEAR_CACHE.size() > 4096) LINEAR_CACHE.clear();
        LINEAR_CACHE.put(key, cached);
        return cached;
    }
    ...
    public Color transfusionEffect(int speed, int index, Color... colors) {
        ...
        int key = (speed << 24) ^ (index << 8) ^ color1.getRGB() ^ (color2.getRGB() << 16);
        Color cached = GRADIENT_CACHE.get(key);
        if (cached != null) return cached;
        cached = ColorUtility.linear(color1, color2, angle / 360F * colors.length - colorIndex);
        if (GRADIENT_CACHE.size() > 8192) GRADIENT_CACHE.clear();
        GRADIENT_CACHE.put(key, cached);
        return cached;
    }
