 ...
 */
public class HandOutlinePipeline {
    ...
    public void render(GpuTextureView target, GpuTextureView scene, GpuTextureView mask,
                       int w, int h, float time, float width, float glow, int mode,
                       float r, float g, float b, float a) {
        if (target == null || scene == null || mask == null || w <= 0 || h <= 0) return;

        ensureInitialized();
        prepareUniformData(w, h, time, width, glow, mode, r, g, b, a);
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "topchdlc:hand_outline", target, OptionalInt.empty())) {
            renderPass.setPipeline(ClientPipelines.HAND_OUTLINE_PIPELINE);
            ...
            renderPass.setUniform("HandOutlineData", uniformBuffer);
            renderPass.draw(0, 6);
        }
    }
    ...
    private void ensureInitialized() {
        ...
        dummyVertexBuffer = createDummyVertexBuffer("topchdlc:hand_outline_dummy_vertex");
    ...
    private void prepareUniformData(int w, int h, float time, float width, float glow, int mode, float r, float g, float b, float a) {
        dataBuffer.clear();
        ...
        dataBuffer.putFloat(glow);
        dataBuffer.putFloat((float) mode);
        dataBuffer.putFloat(0.0f);
        dataBuffer.flip();
    }
    ...
    public void close() {
        if (linearSampler != null) { linearSampler.close(); linearSampler = null; }
